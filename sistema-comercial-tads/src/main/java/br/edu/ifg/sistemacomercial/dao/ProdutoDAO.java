...
public class ProdutoDAO {
    ...
    public void salvar(Produto entity) throws SQLException{
        ...
        String sqlInsert = "insert into produto (nome, marca, codigoBarras, unidadeMedida, categoria_id, id"
                + ") values (?, ?, ?, ?, ?, default)";
        String sqlUpdate = "update produto set nome = ?, marca = ?, codigoBarras = ?, unidadeMedida = ?, categoria_id = ?"
                + " where id = ?";
    ...
    public List<Produto> listar() throws SQLException{
         String sqlQuery = "select p.id, p.nome, p.marca, p.codigoBarras, p.unidadeMedida, c.id as idcategoria, c.nome as nomecategoria from produto as p join categoria as c on c.id = p.categoria_id";
